...
CMagicSkillMng::~CMagicSkillMng()
{
	...
	if (m_pTbl_Type_1 != nullptr)
	{
		delete m_pTbl_Type_1;
		m_pTbl_Type_1 = nullptr;
	}
	if (m_pTbl_Type_2 != nullptr)
	{
		delete m_pTbl_Type_2;
		m_pTbl_Type_2 = nullptr;
	}
	if (m_pTbl_Type_3 != nullptr)
	{
		delete m_pTbl_Type_3;
		m_pTbl_Type_3 = nullptr;
	}
	if (m_pTbl_Type_4 != nullptr)
	{
		delete m_pTbl_Type_4;
		m_pTbl_Type_4 = nullptr;
	}
	//	if(m_pTbl_Type_6 != nullptr) { delete m_pTbl_Type_6; m_pTbl_Type_6 = nullptr; }
	if (m_pTbl_Type_7 != nullptr)
	{
		delete m_pTbl_Type_7;
		m_pTbl_Type_7 = nullptr;
	}
...
{
	if (s_pPlayer->State() == PSA_SPELLMAGIC || s_pPlayer->m_dwMagicID != 0xffffffff || s_pPlayer->m_bStun == true || m_fDelay > 0.0f)
		return true;
	return false;
}
...
bool CMagicSkillMng::HasEquippedRequiredItemGroup(const __TABLE_UPC_SKILL* pSkill) const
{
	...
	if (pSkill->dwNeedItem != 9 && pSkill->dwNeedItem != 0 && pSkill->dwNeedItem != static_cast<uint32_t>(LeftItemGroup)
...
bool CMagicSkillMng::CheckValidCondition(int iTargetID, const __TABLE_UPC_SKILL* pSkill) const
{
	...
	switch (pSkill->iTarget)
	{
		...
		case SKILLMAGIC_TARGET_DEAD_FRIEND_ONLY:
			if (!s_pPlayer->IsHostileTarget(pTarget) && pTarget->IsDead())
			{
				if (bColShape)
				{
					std::string szMsg = fmt::format_text_resource(IDS_SKILL_FAIL_OBJECT_BLOCK);
...
bool CMagicSkillMng::MsgSend_MagicProcess(int iTargetID, __TABLE_UPC_SKILL* pSkill)
{
	...
	switch (pSkill->iTarget)
	{
		...
		case SKILLMAGIC_TARGET_AREA_ALL:
		{
			...
			for (int j = 0; j < 8; j++)
			{
				CGameProcedure::s_pFX->TriggerBundle(s_pPlayer->IDNumber(), 0, FXID_ZONE_POINTER, m_pGameProcMain->m_vMouseSkillPos,
...
bool CMagicSkillMng::CheckValidDistance(const __TABLE_UPC_SKILL* pSkill, const __Vector3& vTargetPos, float fTargetRadius) const
{
	...
	if (pSkill->iValidDist > 0 && fDist <= (pSkill->iValidDist + fTargetRadius + 1.0f))
		return true;
	...
	if (pSkill->dw1stTableType == 2 || pSkill->dw2ndTableType == 2)
	{
		...
		if (pItemIcon2 != nullptr)
			ItemDistance = pItemIcon2->pItemBasic->siAttackRange / 10.0f;
		if (pItemIcon1 != nullptr)
			ItemDistance = pItemIcon1->pItemBasic->siAttackRange / 10.0f;
...
void CMagicSkillMng::StartSkillMagicAtPosPacket(__TABLE_UPC_SKILL* pSkill, const __Vector3& vPos)
{
	...
	if (pSkill->iSelfAnimID1 < 0)
	{
		...
		if (spart2 != 0)
			CGameProcedure::s_pFX->TriggerBundle(SourceID, spart2, pSkill->iSelfFX1, SourceID, spart2, -2);
...
void CMagicSkillMng::UpdateZonePointerPositions()
{
	...
	for (int j = 0; j < 8; j++)
	{
		...
		CGameProcedure::s_pFX->SetBundlePos(FXID_ZONE_POINTER,
...
void CMagicSkillMng::CancelZonePointer()
{
	for (int j = 0; j < 8; j++)
	{
		CGameProcedure::s_pFX->Stop(s_pPlayer->IDNumber(), 0, FXID_ZONE_POINTER, FXID_ZONE_POINTER + j, true);
	}
...
void CMagicSkillMng::SuccessCast(__TABLE_UPC_SKILL* pSkill, CPlayerBase* pTarget)
{
	...
	else
	{
		...
		if (pSkill->dw1stTableType == 2 || pSkill->dw2ndTableType == 2) //화살쏘기..
		{
			...
			__TABLE_UPC_SKILL_TYPE_2* pType2 = m_pTbl_Type_2->Find(pSkill->dwID);
			if (pType2 != nullptr)
				iNumArrow = pType2->iNumArrow;

			idx = AddIdx(pSkill->dwID, iNumArrow);
		}
		else
			idx = AddIdx(pSkill->dwID);
	}
	...
	else
	{
		...
		if (pSkill->iFlyingFX != 0)
		{
			...
			{
				CGameProcedure::s_pFX->TriggerBundle(
					SourceID, spart1, pSkill->iFlyingFX, m_iTarget, 0, idx, FX_BUNDLE_MOVE_DIR_FLEXABLETARGET);
			}
			//
			////////////////////////////////////////////////////
		}
		...
		if (pSkill->iSelfFX2 > 0)
			CGameProcedure::s_pFX->TriggerBundle(
				s_pPlayer->IDNumber(), pSkill->iSelfPart2, pSkill->iSelfFX2, s_pPlayer->IDNumber(), pSkill->iSelfPart2, -3);
	}
}
...
void CMagicSkillMng::ProcessCasting()
{
	...
	if (s_pPlayer->m_dwMagicID != 0xffffffff)
	{
		...
		if (pTarget != nullptr)
			s_pPlayer->RotateTo(pTarget); // 일단 타겟을 향해 방향을 돌린다..
...
void CMagicSkillMng::MsgRecv_Casting(Packet& pkt)
{
	...
	if (spart2 != 0)
		CGameProcedure::s_pFX->TriggerBundle(iSourceID, spart2, pSkill->iSelfFX1, iSourceID, spart2, -2);
...
void CMagicSkillMng::MsgRecv_Flying(Packet& pkt)
{
	...
	{
		CGameProcedure::s_pFX->TriggerBundle(
			iSourceID, spart1, pSkill->iFlyingFX, iTargetID, 0 /*pSkill->iTargetPart*/, Data[3], FX_BUNDLE_MOVE_DIR_FLEXABLETARGET);
	}
}
...
void CMagicSkillMng::MsgRecv_Effecting(Packet& pkt)
{
	...
	{
		if (iTargetID == s_pPlayer->IDNumber())
			EffectingType4(dwMagicID);
	}

	if (pSkill->dw1stTableType == 3 || pSkill->dw2ndTableType == 3)
	{
		if (iTargetID == s_pPlayer->IDNumber())
			EffectingType3(dwMagicID);
	}

	if (pSkill->iFlyingFX != 0 && (pSkill->iTarget < SKILLMAGIC_TARGET_AREA_ENEMY || pSkill->iTarget > SKILLMAGIC_TARGET_AREA))
		return; //플라잉이 있는 마법의 경우는 효과를 fail에서 처리한다..
...
void CMagicSkillMng::MsgRecv_Fail(Packet& pkt)
{
	...
	if (Data[3] == SKILLMAGIC_FAIL_ATTACKZERO)
	{
		...
		if (iSourceID == s_pPlayer->IDNumber())
		{
			...
			std::string szMsg = fmt::format_text_resource(IDS_MSG_FMT_TARGET_ATTACK_FAILED, pSkill->szName);
	...
	if (Data[3] == SKILLMAGIC_FAIL_KILLFLYING) //flying효과 죽이고..그자리에 타겟효과 해라..
	{
			...
			|| ((iTargetID == s_pPlayer->IDNumber() && s_pOPMgr->NPCGetByID(iSourceID, false) != nullptr)))
		{
			...
			CGameProcedure::s_pFX->Stop(iSourceID, iTargetID, pSkill->iFlyingFX, Data[4]);
...
void CMagicSkillMng::FlyingType2(__TABLE_UPC_SKILL* pSkill, int iSourceID, int iTargetID, int16_t* pData)
{
	...
	if (pTarget == nullptr)
	{
		...
		CGameProcedure::s_pFX->TriggerBundle(
			iSourceID, spart1, pSkill->iFlyingFX, vTargetPos + pPlayer->Position(), pData[3], pType2->iSuccessType);
		...
		for (int i = 1; i <= NumArrow; i++)
		{
			...
			CGameProcedure::s_pFX->TriggerBundle(
				iSourceID, spart1, pSkill->iFlyingFX, vTargetPos3 + pPlayer->Position(), idx++, pType2->iSuccessType);
	...
	else
	{
		...
		{
			CGameProcedure::s_pFX->TriggerBundle(
				iSourceID, spart1, pSkill->iFlyingFX, vTargetPos, pData[3], FX_BUNDLE_MOVE_DIR_FIXEDTARGET);
			...
			for (int i = 1; i <= NumArrow; i++)
			{
				...
				vTargetPos3 = vTargetPos2 * mtx;
				CGameProcedure::s_pFX->TriggerBundle(
					iSourceID, spart1, pSkill->iFlyingFX, vTargetPos3 + pPlayer->Position(), idx++, FX_BUNDLE_MOVE_DIR_FIXEDTARGET);
		...
		{
			CGameProcedure::s_pFX->TriggerBundle(
				iSourceID, spart1, pSkill->iFlyingFX, iTargetID, pSkill->iTargetPart, pData[3], pType2->iSuccessType);
...
void CMagicSkillMng::EffectingType3(uint32_t dwMagicID)
{
	...
	if (key == DDTYPE_TYPE3_DUR_ENEMY && pType3->iAttribute == 3)
		StopCastingByRatio();

	if (pType3->iDurationTime == 0)
		return;
